...
# To be reviewed
class hierarchical_likelihood_neffPE(bilby.Likelihood):
    def __init__(self, posterior_samples_dict, injections, rate_model, nparallel=None, neffPE=20,neffINJ=None,likelihood_variance_thr=None):
        '''
        Base class for an hierachical likelihood. It just saves all the input requirements for a general hierarchical analysis.

        Notes that this class proposes a more flexible condition for neffPE, under which a model must have more than 20% of incompatible 
        events to be rejected, rather than just a single event in the base class.
    ...
    def log_likelihood(self):
        ...
        # Base condition of NeffPE
        cond_neffPE = (neff_PE_ev < self.neffPE)
        # Number of events rejected by this condition
        Nb_under = np.sum(cond_neffPE, axis=0)
        
        #if more than 20% of the events are rejected, reject the parameter space point
        if (Nb_under > 20.0/100.0*self.posterior_samples_dict.n_ev) | xp.any(neff_PE_ev==0.):
            return float(xp.nan_to_num(-xp.inf))
